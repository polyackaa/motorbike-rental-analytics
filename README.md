...
Проект представляет собой полноценную базу данных для гипотетической компании по прокату мототехники. Помимо структуры БД, проект включает набор готовых потенциальных аналитических запросов и визуализацию ключевых метрик на Python.
...
# 3. Скопировать файл `.env.example` в `.env` и заполнить своими данными (для подключения к БД):
cp .env.example .env

# 4. Запустить код для проведения запросов и их визуализации
python main.py
```
...
```bash
├── main.py           # код для проведения SQL-запросов и визуализации результатов
